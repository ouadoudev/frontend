...
const Profile = () => {
  ...
  const { error: userError, status: userStatus, user: detailedUser } = useSelector((state) => state.user);
  const { todos, status: todosStatus } = useSelector((state) => state.todos);
  ...
  useEffect(() => {
    if (authUser?.id && typeof authUser.id === "string" && authUser.id.length > 0) {
      ...
      <div className="w-full min-h-screen">
        <div className="container mx-auto mt-8 px-4 md:px-6">
          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Error fetching user data: {typeof userError === "string" ? userError : userError?.message || "Unknown error"}
            </AlertDescription>
          </Alert>
        </div>
      </div>
