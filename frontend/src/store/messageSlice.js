...
export const createMessage = createAsyncThunk(
  ...
  async ({ conversationId, senderId, content, images, videos, pdfs }, { rejectWithValue }) => {
    try {
...
const messageSlice = createSlice({
  ...
  reducers: {
    ...
    replaceOptimisticMessage: (state, action) => {
      ...
      const index = state.messages.findIndex((message) => message._id === oldId)
      if (index !== -1) {
  ...
  extraReducers: (builder) => {
    builder
      // Removed the createMessage.pending case
...
export const { addMessage, replaceOptimisticMessage, updateMessageStatus, editMessage, deleteMessage } = messageSlice.actions
