...
const Lesson = () => {
  ...
  const handleDownload = async () => {
    try {
      const response = await fetch(lesson.pdf.url.replace(/^http:/, "https:"), {
        mode: "cors",
      });
            ...
            <Card className="mb-6 border-hidden">
              ...
              <CardContent className="pt-4">
                ...
                <div className="space-y-2">
                  {lesson.pdf && lesson.pdf.url ? (
                    <div className="mt-6 border border-gray-300 rounded-md shadow-sm bg-white">
                      <ScrollArea className="h-[80vh] p-4 bg-gray-50 rounded-md">
                        <Document
                          file={lesson.pdf.url.replace(/^http:/, "https:")}
                          onLoadSuccess={({ numPages }) => setNumPages(numPages)}
                          loading={<p>Chargement du PDF...</p>}
                          error={<p>Échec du chargement du PDF.</p>}
                        >
                          {Array.from(new Array(numPages), (_, index) => (
                            <Page
                              key={`page_${index + 1}`}
                        ...
                        >
                          <Card className="flex flex-col">
                            ...
                            <CardContent className="flex-grow p-4">
                              ...
                              <span className="text-xs">
                                {formatDuration(relatedLesson.video.duration)} min
                              </span>
                            </CardContent>
                          </Card>
                        </CarouselItem>
