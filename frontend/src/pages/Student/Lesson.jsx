...
const Lesson = () => {
  ...
  useEffect(() => {
    const handleScroll = async () => {
      if (videoRef.current && !isPiPActive && videoRef.current.paused === false && window.scrollY > 200) {
        try {
          await videoRef.current.requestPictureInPicture();
          setIsPiPActive(true);
        } catch (error) {
  ...
  const handleCompleteLesson = async () => {
    try {
      await dispatch(updateProgress({ courseId: lesson.course._id, lessonId: id }));
      alert("Lesson marked as completed!");
    } catch (error) {
      console.error("Error updating progress:", error);
    }
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
            <div className="space-y-4">
              ...
              <h2 className="text-2xl text-center font-bold">{lesson.title}</h2>
                ...
                <div className="flex items-center justify-between">
                  <Button
                    className="w-40 h-8 bg-blue-500 rounded-xl text-white hover:bg-blue-600"
                    onClick={handleCompleteLesson}
                  >
                    Complete Lesson
                  </Button>
                  <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger>
                      <Button className="w-40 h-8 bg-blue-500 rounded-xl text-white hover:bg-blue-600">
                        Open Quiz
                      </Button>
                    </DialogTrigger>
                    <DialogContent>
                  ...
                  {relatedLessons && relatedLessons.length > 0 ? (
                    <div className="flex flex-col gap-4">
                      {relatedLessons.map((relatedLesson) => (
                        <div
                          key={relatedLesson._id}
                          className="relative flex items-center gap-4 w-full rounded-lg border border-gray-200 p-2 dark:border-gray-800 group"
                        >
                          <img
