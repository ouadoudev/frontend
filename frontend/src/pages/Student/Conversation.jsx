...
// import { fetchUserConversations, setCurrentConversation, updateLastMessageRedux } from "@/store/conversationSlice"
// import { addMessage, fetchMessages, replaceOptimisticMessage, updateMessageStatus } from "@/store/messageSlice"
// import {
//   PhoneIcon,
//   SearchIcon,
//   VideoIcon,
...
//   const messagesLoading = useSelector((state) => state.messages.status === "loading")
...
//         if (currentConversation._id && message.conversationId === currentConversation._id) {
//           dispatch(addMessage({ ...message}))
//         }
//       }

//       const handleUpdateLastMessage = ({ conversationId, lastMessage, lastMessageId }) => {
//         dispatch(updateLastMessageRedux({ conversationId, lastMessage, lastMessageId }))
//       }
...
//       }
//     }
//   }, [currentConversation?._id, dispatch, userId])
...
//       })
//     }, 2000)
//   }, [currentConversation, userId, user?.username, isTyping])
...
//       setConnectionError("Some files were rejected. Only images, videos, and PDFs under 20MB are allowed.")
//       setTimeout(() => setConnectionError(null), 5000)
//     }
...
//       setConnectionError(error.response?.data?.message || "Failed to send message. Please try again.")
//       setIsUploading(false)
...
//     }
//   }, [newMessage, selectedFiles, currentConversation, userId, user, isConnected, dispatch])
...
//         <div
//           className={`fixed inset-y-0 left-0 z-50 w-80 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:z-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
//         >
//           <div className="h-screen bg-sidebar border-r border-sidebar-border flex flex-col">
//             <div className="p-4 border-b border-sidebar-border">
//               <div className="flex items-center justify-between mb-4">
//                 <h2 className="font-heading font-bold text-xl text-sidebar-foreground">Messages</h2>
//                 <div className="flex items-center gap-2">
//                   <Tooltip>
...
//                   <Button size="sm" variant="ghost" className="text-sidebar-foreground hover:bg-sidebar-accent">
...
//                       className={`p-4 border-b border-sidebar-border cursor-pointer transition-colors rounded-lg mb-1 hover:bg-sidebar-accent ${isActive ? "bg-sidebar-accent" : ""}`}
...
//                             <AvatarImage
//                               src={participant?.user_image?.url || "/placeholder.svg"}
...
//                     <AvatarImage src={user?.user_image?.url || "/placeholder.svg"} alt="You" />
//                     <AvatarFallback className="bg-primary text-primary-foreground">
...
//                 </div>
//                 <div className="flex-1 min-w-0">
//                   <p className="font-medium text-sidebar-foreground">{user?.username || "You"}</p>
...
//             </Button>
//             <h1 className="font-heading font-bold text-lg text-foreground">ChatApp</h1>
//             <div className="w-10" />
//           </div>
...
//                     <div className="flex items-center gap-2">
//                       {!isConnected && !isConnecting && (
//                         <Button variant="ghost" size="sm" onClick={handleForceReconnect} className="text-destructive">
...
//                       <Avatar className="h-10 w-10">
//                         <AvatarImage
//                           src={currentConversation.participants?.[1]?.user_image?.url || "/placeholder.svg"}
//                           alt={currentConversation.participants?.[1]?.username || "User"}
//                         />
//                         <AvatarFallback className="bg-primary text-primary-foreground">
//                           {currentConversation.participants?.[1]?.username?.[0]?.toUpperCase() || "U"}
//                         </AvatarFallback>
//                       </Avatar>
//                       {onlineUsers[currentConversation.participants?.[1]?._id] && (
...
//                     <Tooltip>
//                       <TooltipTrigger asChild>
//                         <Button variant="ghost" size="icon" disabled={!isConnected} className="text-card-foreground">
...
//                         <div
//                           key={message._id}
//                           className={`flex gap-3 ${message.senderId._id === userId ? "flex-row-reverse" : "flex-row"}`}
//                         >
//                           <Avatar className="h-8 w-8 flex-shrink-0">
//                             <AvatarImage
//                               src={
//                                 message.senderId._id === userId
//                                   ? user?.user_image?.url || "/placeholder.svg"
...
//                           <div
//                             className={`flex flex-col max-w-xs sm:max-w-md ${message.senderId._id === userId ? "items-end" : "items-start"}`}
//                           >
//                             <div
//                               className={`px-4 py-2 rounded-lg text-sm leading-relaxed ${message.senderId._id === userId ? "bg-primary text-primary-foreground" : "bg-card text-card-foreground border border-border"}`}
//                             >
//                               {message.content && <p>{message.content}</p>}

//                               {message.media?.map((media) => (
//                                 <div key={media._id || media.url} className="mt-2">
//                                   {media.type === "image" ? (
//                                     <div
//                                       className="relative cursor-pointer group"
//                                       onClick={() =>
//                                         setSelectedMedia({ url: media.url || "/placeholder.svg", type: "image" })
//                                       }
//                                     >
//                                       <img
//                                         src={media.url || "/placeholder.svg"}
//                                         alt="Shared image"
//                                         className="max-w-full h-auto rounded-lg transition-all duration-200 group-hover:opacity-90 group-hover:scale-[1.02]"
...
//                                     >
//                                       <video
//                                         className="max-w-full h-auto rounded-lg transition-all duration-200 group-hover:scale-[1.02]"
//                                         style={{ maxHeight: "200px" }}
//                                         preload="metadata"
...
//                                       <a href={media.url} target="_blank" rel="noopener noreferrer" className="text-sm">
//                                         {media.type === "pdf" ? "PDF Document" : media.type}
//                                       </a>
//                                     </div>
//                                   )}
//                                 </div>
...
//                               </span>
//                               {message.senderId._id === userId && <div>{getMessageStatusIcon(message.status)}</div>}
//                             </div>
//                           </div>
//                         </div>
//                       ))}
...
//                 <h3 className="text-lg font-semibold mb-2 text-foreground">Welcome to Messages</h3>
                          ...
                          <div className="flex items-start gap-3">
                            <div className="relative">
                              <Avatar className="h-12 w-12 flex-shrink-0">
                                ...
                                />
                                <AvatarFallback>
                                  {participant?.username?.[1]?.toUpperCase() ||
                                    "?"}
                                </AvatarFallback>
                              </Avatar>
                              {isOnline && (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <Avatar className="h-10 w-10">
                            <AvatarImage
                              src={
                                currentConversation.participants?.[1]
                                  ?.user_image?.url ||
                                "/placeholder.svg" ||
                                "/placeholder.svg" ||
                                "/placeholder.svg" ||
                            ...
                            />
                            <AvatarFallback className="bg-primary text-primary-foreground">
                              {currentConversation.participants?.[1]?.username?.[1]?.toUpperCase() ||
                                "U"}
                            </AvatarFallback>
                          </Avatar>
                          {onlineUsers[
                            currentConversation.participants?.[1]?._id
                          ] && (
                            <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 border-2 border-card rounded-full"></div>
                          )}
                        </div>
