...
//   // Inside Exercise.jsx, within the ExerciseGenerator component:
...
import { Progress } from "@/components/ui/progress";
import { Switch } from "@/components/ui/switch";
// Note : Si vous utilisez React 18 et que le Drag&Drop ne marche pas, 
// remplacez "react-beautiful-dnd" par "@hello-pangea/dnd" (il faudra faire: npm i @hello-pangea/dnd)
import { DragDropContext, Droppable, Draggable } from "react-beautiful-dnd";
import {
  Plus,
  Trash2,
...
const ExerciseGenerator = () => {
  ...
  const addQuestion = (type) => {
    const newQuestion = {
      ...
      questionText: "",
      difficulty: 1,
      // Correction: correctAnswer (string) pour multiple-choice, correctAnswers (array) pour les autres
      options: type === "multiple-choice" ? [""] : undefined,
  ...
  const handleSubmit = async (e) => {
    if (e && e.preventDefault) e.preventDefault(); // Sécurité
    ...
    setIsSubmitting(true);
    try {
      // Correction: Ajout de .unwrap() pour intercepter les erreurs API
      const resultAction = await dispatch(
        createExercise({ ...exercise, lessonId }),
      ).unwrap();

      // Selon la structure de votre backend
      const newExerciseId = resultAction.exercise._id;
            ...
            <div className="mt-4">
              ...
              {question.dragAndDrop?.correctOrder?.map((order, index) => (
                <div key={index} className="flex items-center space-x-2 mt-2">
                  <span>{index + 1}.</span>
                  {/* Correction : Remplacement du <select> natif par le composant <Select> Shadcn */}
                  <div className="flex-1 max-w-[300px]">
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        // Récupère la vraie valeur même si on a utilisé un fallback "empty_X"
                        newCorrectOrder[index] = value.startsWith("empty_") ? "" : value;
                        updateQuestion(question.id, {
                          dragAndDrop: {
                            ...question.dragAndDrop,
                            correctOrder: newCorrectOrder,
                      ...
                      <SelectContent>
                        {question.dragAndDrop?.items?.map((item, i) => {
                          ...
                          return (
                            <SelectItem 
                              key={i} 
                              // Shadcn SelectItem plante si la valeur est vide, on ajoute un fallback
                              value={item || `empty_${i}`} 
                              disabled={isSelectedElsewhere && item !== ""}
                            >
                              {item || `Élément ${i + 1}`}
                            </SelectItem>
                          );
              ...
              <div className="space-y-4">
                ...
                {exercise.questions.map((question, index) => (
                  ...
                  >
                    ...
                    {/* Correction: Aperçu QCM fonctionnel */}
                    {question.type === "multiple-choice" && (
                      <div className="space-y-1">
                        {question.options?.map((option, optIndex) => (
                          <div
    ...
    <div className="min-h-screen">
      <div className="container mx-auto py-8">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {/* Step Content */}
          <form
            // Correction : L'event 'e' est maintenant passé
            onSubmit={(e) => {
              e.preventDefault();
              if (currentStep === STEPS.length) {
                handleSubmit(e);
