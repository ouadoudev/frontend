...
import { createAnnualPlan, clearPlanningState } from '../../../store/planningSlice';
// Assumez que vous avez un slice pour récupérer les programmes officiels
import { fetchPrograms } from '../../../store/programSlice';
import { 
  Calendar, Clock, MapPin, BookOpen, Plus, 
  Trash2, Sparkles, ChevronRight, AlertCircle 
} from 'lucide-react';
...
const CreatePlanPage = () => {
  ...
  const addScheduleSlot = () => {
    setFormData({
      ...formData,
      weeklySchedule: [...formData.weeklySchedule, { day: 1, startTime: '14:00', endTime: '16:00', durationHours: 2 }]
    });
  };
  ...
  const updateSlot = (index, field, value) => {
    ...
    newSchedule[index][field] = field === 'day' || field === 'durationHours' ? parseInt(value) : value;
    setFormData({ ...formData, weeklySchedule: newSchedule });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.programId) return toast.error("Veuillez choisir un programme");
    ...
    <div className="min-h-screen bg-slate-50 pb-20 pt-10 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">Assistant de Planification</h1>
          <p className="text-slate-500 mt-2 font-medium text-lg">Générez votre année scolaire 2025/2026 en quelques secondes.</p>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 ml-1">Programme Officiel</label>
                <select 
                ...
                >
                  ...
                  {programs?.map(p => (
                    <option key={p._id} value={p._id}>{p.title} ({p.educationalLevel})</option>
                  ))}
                </select>
              </div>

              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 ml-1">Région (Vacances)</label>
                <select 
                  className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-4 py-3 outline-none focus:border-blue-500 focus:bg-white transition-all text-sm font-bold"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 ml-1">Date de la rentrée</label>
                <input 
                  type="date" 
                  className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-4 py-3 outline-none focus:border-blue-500 focus:bg-white transition-all text-sm font-bold"
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 ml-1">Fin de l'année</label>
                <input 
                  type="date" 
                  className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-4 py-3 outline-none focus:border-blue-500 focus:bg-white transition-all text-sm font-bold"
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="space-y-4">
              {formData.weeklySchedule.map((slot, index) => (
                <div key={index} className="flex flex-col md:flex-row gap-4 items-end bg-slate-50 p-6 rounded-[2rem] border border-slate-100 relative group">
                  <div className="flex-1 w-full space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Jour</label>
                    <select 
                  ...
                  <div className="w-full md:w-24 space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Durée (H)</label>
                    <input 
                      type="number" 
                      className="w-full bg-white border-none rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm"
                      value={slot.durationHours}
                      onChange={(e) => updateSlot(index, 'durationHours', e.target.value)}
                    />
                  </div>
