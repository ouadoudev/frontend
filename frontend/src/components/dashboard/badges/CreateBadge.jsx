...
import { createBadge } from "@/store/badgeSlice";
...
const conditionTypes = [
  "quiz_count", "quiz_score", "quiz_time", "course_completion",
  "lesson_completion", "time_spent", "testimonial_given", "exam_score",
  "subject_enrollment", "custom_field",
];
...
const CreateBadge = () => {
  ...
  const categoryOptions = ["performance", "progress", "engagement", "social", "special"];
const iconOptions = [
  ...
  const handleConditionChange = (index, name, value) => {
    ...
    if (errors[`condition-${index}-${name}`]) {
      setErrors((prev) => ({ ...prev, [`condition-${index}-${name}`]: "" }));
    }
  };
  ...
  const addCondition = () => {
    setFormData((prev) => ({
      ...prev,
      conditions: [...prev.conditions, { type: "", field: "", operator: "", value: "", description: "" }],
    }));
  };
  ...
  const removeCondition = (index) => {
    ...
    setErrors((prev) => {
      ...
      Object.keys(newErrors).forEach((key) => {
        if (key.startsWith(`condition-${index}-`)) delete newErrors[key];
      });
      return newErrors;
    });
  };
  ...
  const addPrerequisite = () => {
    ...
    if (!trimmedInput) {
      toast.error("Prerequisite badge ID is required.");
      return;
    }
    if (formData.prerequisites.includes(trimmedInput)) {
      toast.error("This badge ID is already added.");
      return;
    }
    setFormData((prev) => ({
      ...prev,
  ...
  const validateForm = () => {
    ...
    if (!formData.badgeId.trim()) newErrors.badgeId = "Badge ID is required.";
    if (!formData.description.trim()) newErrors.description = "Description is required.";
    if (!formData.category.trim()) newErrors.category = "Category is required.";
    if (formData.points < 1) newErrors.points = "Points must be at least 1.";
    if (formData.maxEarnings < 1) newErrors.maxEarnings = "Max earnings must be at least 1.";
    ...
    formData.conditions.forEach((condition, index) => {
      if (!condition.type) newErrors[`condition-${index}-type`] = "Condition type is required.";
      if (!condition.field) newErrors[`condition-${index}-field`] = "Field is required.";
      if (!condition.operator) newErrors[`condition-${index}-operator`] = "Operator is required.";
      if (!condition.value) newErrors[`condition-${index}-value`] = "Value is required.";
      if (!condition.description) newErrors[`condition-${index}-description`] = "Description is required.";
      if (["==", "!=", ">=", "<=", ">", "<"].includes(condition.operator) && isNaN(condition.value)) {
        newErrors[`condition-${index}-value`] = "Value must be a number for this operator.";
      }
    });
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="badgeId">BadgeId *</Label>
                  <Input
                    id="badgeId"
                    name="badgeId"
                    ...
                    aria-invalid={!!errors.badgeId}
                    aria-describedby={errors.badgeId ? "name-error" : undefined}
                  />
                  {errors.badgeId && (
                    <p id="badgeId-error" className="text-red-500 text-sm mt-1">
                      {errors.badgeId}
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    aria-describedby={errors.maxEarnings ? "maxEarnings-error" : undefined}
                  />
                  {errors.maxEarnings && (
                    <p id="maxEarnings-error" className="text-red-500 text-sm mt-1">
                      {errors.maxEarnings}
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <BadgeIcon iconName={formData.icon} className="w-6 h-6" style={{ color: formData.color }} />
                  </div>
                </div>
                <div>
              ...
              <div className="space-y-4 pt-6 border-t">
                ...
                <p className="text-sm text-gray-500">Define the criteria for earning the badge.</p>
                {formData.conditions.map((condition, index) => (
                  <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-3">
                    <div className="flex justify-between items-center">
                      <h4 className="font-semibold text-gray-800">Condition #{index + 1}</h4>
                      <Button
                        type="button"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm ${
                            errors[`condition-${index}-type`] ? "border-red-500" : ""
                          }`}
                          aria-invalid={!!errors[`condition-${index}-type`]}
                          aria-describedby={errors[`condition-${index}-type`] ? `condition-type-error-${index}` : undefined}
                        >
                        ...
                        {errors[`condition-${index}-type`] && (
                          <p id={`condition-type-error-${index}`} className="text-red-500 text-sm mt-1">
                            {errors[`condition-${index}-type`]}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        <select
                          ...
                          className={`w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm ${
                            errors[`condition-${index}-operator`] ? "border-red-500" : ""
                          }`}
                          aria-invalid={!!errors[`condition-${index}-operator`]}
                          aria-describedby={errors[`condition-${index}-operator`] ? `condition-operator-error-${index}` : undefined}
                        >
                        ...
                        {errors[`condition-${index}-operator`] && (
                          <p id={`condition-operator-error-${index}`} className="text-red-500 text-sm mt-1">
                            {errors[`.condition-${index}-operator`]}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          className={errors[`condition-${index}-field`] ? "border-red-500" : ""}
                          aria-invalid={!!errors[`condition-${index}-field`]}
                          aria-describedby={errors[`condition-${index}-field`] ? `condition-field-error-${index}` : undefined}
                        />
                        {errors[`condition-${index}-field`] && (
                          <p id={`condition-field-error-${index}`} className="text-red-500 text-sm mt-1">
                            {errors[`condition-${index}-field`]}
                          </p>
                        )}
                      </div>
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="e.g., 90"
                          className={errors[`condition-${index}-value`] ? "border-red-500" : ""}
                          aria-invalid={!!errors[`condition-${index}-value`]}
                          aria-describedby={errors[`condition-${index}-value`] ? `condition-value-error-${index}` : undefined}
                        />
                        {errors[`condition-${index}-value`] && (
                          <p id={`condition-value-error-${index}`} className="text-red-500 text-sm mt-1">
                            {errors[`condition-${index}-value`]}
                          </p>
                        )}
                      </div>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className={errors[`condition-${index}-description`] ? "border-red-500" : ""}
                        aria-invalid={!!errors[`condition-${index}-description`]}
                        aria-describedby={errors[`condition-${index}-description`] ? `condition-description-error-${index}` : undefined}
                      />
                      {errors[`condition-${index}-description`] && (
                        <p id={`condition-description-error-${index}`} className="text-red-500 text-sm mt-1">
                          {errors[`condition-${index}-description`]}
                        </p>
                      )}
                    </div>
              ...
              <div className="space-y-4 pt-6 border-t">
                ...
                <p className="text-sm text-gray-500">Other badges required to earn this one. Enter a Badge ID.</p>
                <div className="flex gap-2">
                  <Input
                    type="text"
                  ...
                  />
                  <Button type="button" onClick={addPrerequisite} aria-label="Add prerequisite">
                    Add
                  </Button>
                </div>
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.prerequisites.map((prereq) => (
                    <div key={prereq} className="flex items-center bg-gray-200 text-gray-800 rounded-full px-3 py-1 text-sm font-medium">
                      <span>{prereq}</span>
                      <button
                        type="button"
                 ...
                 <div>
                   ...
                   <Input
                     ...
                     value={formData.availableFrom}
                      onChange={handleInputChange}
                     className="w-full"
                     aria-describedby={errors.availableUntil ? "availableUntil-error" : undefined}
                   />
                 </div>
                 <div>
                   ...
                   {errors.availableUntil && (
                     <p id="availableUntil-error" className="text-red-500 text-sm mt-1">
                      {errors.availableUntil}
                     </p>
                   )}
                 </div>
             ...
             <div className="flex justify-end gap-3 pt-6 border-t">
              <Button
                ...
                onClick={handleCancel}
                 disabled={isLoading}
                 aria-label="Cancel creating badge"
               >
                 Cancel
               </Button>
               <Button
               ...
               className="flex items-center gap-2"
                  aria-label={isLoading ? "Creating badge" : "Create badge"}
                >
                 {isLoading ? <Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" /> : <Save className="w-4 h-4" />}
